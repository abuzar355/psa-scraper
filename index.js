...
function generateCSVAndSend(data, socketId) {
  try {
    ...
    io.to(socketId).emit("log", {
      message: `Error generating CSV file. ${err.message}`,
    });
  }
}
...
async function scrapeData(set_name, grade_value, socketId, data) {
  ...
  await page.click(
    "table.w-full.border.border-neutralstroke2 tbody tr:first-child td:first-child a"
  );
  ...
  for (let link of breadcrumbLinks) {
    ...
    if (text.toLowerCase().includes(set_name.toLowerCase())) {
      await link.click();
      io.to(socketId).emit("log", {
        message: `Clicked on the breadcrumb link for '${set_name}'.`,
      });
      found = true;
      break;
    }
  ...
  if (!found) {
    io.to(socketId).emit("log", {
      message: `Set name '${set_name}' not found in breadcrumbs.`,
    });
  }
  ...
  for (let option of gradeOptions) {
    ...
    if (text.toLowerCase() === grade_value.toLowerCase()) {
      await option.click();
      io.to(socketId).emit("log", {
        message: `Selected grade: ${grade_value}.`,
      });
      gradeFound = true;
      break;
    }
  ...
  if (!gradeFound) {
    io.to(socketId).emit("log", {
      message: `Grade '${grade_value}' not found in dropdown.`,
    });
  }
  ...
  for (const row of data) {
    ...
    if (!player_name || processedPlayers.has(player_name)) {
      io.to(socketId).emit("log", {
        message: `Skipping row due to missing player name or already processed: ${
    ...
    try {
      ...
      if (searchBox) {
        ...
        io.to(socketId).emit("log", {
          message: `Found ${rows.length} results.`,
        });
      } else {
        ...
        const playerWords = player_name
          .split(" ")
          .map((word) => word.toLowerCase());
        let filteredRows = [];
        for (let row of rows) {
          const rowText = await row.$eval("td:nth-child(2) a", (el) =>
        ...
        if (filteredRows.length > 0) {
          io.to(socketId).emit("log", {
            message: `Found matching rows for player: ${player_name}`,
          });
          rows = filteredRows; // Replace rows with filtered rows
        } else {
          io.to(socketId).emit("log", {
            message: `No matching rows found for player: ${player_name}`,
          });
        }
      }
      ...
      if (rows.length > 0) {
        ...
        const existingPlayerIndex = data.findIndex(
          (item) => item["Player"] === player_name
        );

        if (existingPlayerIndex !== -1) {
          for (let row of rows) {
            const number = await row.$eval("td:nth-child(1)", (el) =>
              el.textContent.trim()
            );
            const recent_price = await row.$eval("td:nth-child(3)", (el) =>
              el.textContent.trim()
            );
            ...
            if (first_row) {
              ...
              io.to(socketId).emit("log", {
                message: `Updating row for Player: ${player_name}, Number: ${number}`,
              });
              ...
              io.to(socketId).emit("log", {
                message: `Inserting row for Player: ${player_name}, Number: ${number}`,
              });
            }
          }
        }
      ...
      io.to(socketId).emit("log", {
        message: `Error processing player ${player_name}: ${error.message}`,
      });
    }
  }
...
app.post("/extract-player", upload.array("fileInput"), async (req, res) => {
  ...
  try {
    ...
    io.to(socketId).emit("log", {
      message: `Total Image Files: ${req.files.length}.`,
    });
    ...
    io.to(socketId).emit("log", {
      message: `Processing in batches of ${batchSize}. Total Batches: ${imageBatches.length}.`,
    });

    // Loop through each batch
    for (const [index, batch] of imageBatches.entries()) {
      io.to(socketId).emit("log", {
        message: `Processing Batch ${index + 1} of ${imageBatches.length}.`,
      });
      const messages = [
        {
          role: "user",
      ...
      io.to(socketId).emit("log", {
        message: `Sending Batch ${index + 1} to GPT-4.`,
      });
      ...
      io.to(socketId).emit("log", {
        message: `Batch ${index + 1} processed. Extracting player names.`,
      });
      ...
      io.to(socketId).emit("log", {
        message: `Batch ${index + 1} completed. Player names extracted.`,
      });
    ...
    io.to(socketId).emit("log", {
      message: "Generating a shareable link for the Google Sheet.",
    });

    // Generate a shareable link
    const webViewLink = await generateShareableLink(
      google.drive({ version: "v3", auth: authClient }),
      spreadsheetId
    );
    io.to(socketId).emit("log", {
      message: `Player names added to Google Sheet. Link: ${webViewLink}`,
    });
    ...
    res
      .status(200)
      .json({ message: "Player name extraction complete!", link: webViewLink });
  } catch (error) {
